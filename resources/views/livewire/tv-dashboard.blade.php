    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
            <div class="w-20 h-10 flex items-center justify-center">
                            <img src="{{ asset('images/logo_ugm_putih.png') }}" alt="Logo UGM" class="h-19 w-auto">
                        </div>
            <div>
        ...
        <div class="text-right">
            ...
            <p class="text-lg text-blue-200">{{ now()->format('d F Y') }}</p>
        </div>
    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    @foreach($currentCalls as $call)
                        <div class="called-card bg-surface rounded-2xl p-8 text-center border-4 border-secondary">
                            <p class="text-lg text-text-secondary mb-2">{{ $call->counter->name ?? 'Loket' }}</p>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    @foreach($services as $service)
                        <div class="bg-white/10 rounded-lg p-3 text-center">
                            <p class="text-sm text-blue-200">{{ $service->name }}</p>
